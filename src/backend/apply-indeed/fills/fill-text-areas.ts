...
export async function fillTextAreas(page: Page,  model: GenerativeModel) {
  ...
  if (textAreas.length) {
    for (const textTextArea of textAreas) {
      ...
      if (textAreaValue === "") { //to-do: caso q n exista
        ...
        const foundFieldKey = Object.keys(knownFields).find((key) =>
          knownFields[key as keyof typeof knownFields].some((fieldValue) =>
            associatedLabel?.includes(fieldValue)
          )
